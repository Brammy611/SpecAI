...
exports.fetchRepositoryContent = fetchRepositoryContent;
 ...
 */
async function fetchRepositoryContent(repoUrl, githubToken) {
    ...
    const textFiles = (treeData.tree || []).filter((item) => item.type === "blob" &&
        item.path &&
    ...
    for (let i = 0; i < textFiles.length; i += CONCURRENCY) {
        ...
        const fetched = await Promise.allSettled(batch.map(async (item) => {
