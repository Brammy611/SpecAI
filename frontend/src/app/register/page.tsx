...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || `Registrasi gagal (${res.status}).`);
      }
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col font-sans">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-[460px] bg-white rounded-[24px] border border-gray-200 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 md:p-8">
          <h1 className="text-2xl font-jakarta font-bold text-gray-900">Register</h1>
          <p className="mt-1 text-sm text-gray-600">
            Buat akun untuk menyimpan histori analisis.
          </p>
          ...
          <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
            ...
            <p className="text-sm text-gray-600 text-center">
              Sudah punya akun?{" "}
              <Link href="/login" className="font-semibold text-[#FF6B00] hover:text-[#E66000]">
                Login
              </Link>
            </p>
          </form>
